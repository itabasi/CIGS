...
extern double GetRad(int run, int ch);
extern int GetRunNum(string rname);
const int nch =8;
...
void CIGS_MGy(){ // Current timde dependence
  ...
  for(int i =0; i<2; i++){
	...
	if(ifS1.eof() || buf.size()==0)break;
      ...
      //      cout<<"ii "<<iii<<" SCALE0 "<<SCALE0[iii]<<" SCALE1 "<<SCALE1[iii]<<" SCALE2 "<<SCALE2[iii]<<" SCALE3 "<<SCALE3[iii]<<" SCALE4 "<<SCALE4[iii]<<endl;
  ...
  // =========== Get Luminosity Info ==================== //
  //string lumi_list="/home/togawa/work/SENSOR/CIGS/Ana/analysis/itamacro/runlist/luminosity.txt";
  string lumi_list="/home/togawa/work/SENSOR/CIGS/Ana/analysis/itamacro/runlist/luminosity_all.txt";
  ifstream ifLL(lumi_list.c_str(),ios::in); // IT runlist
  int lumi_i =0;
  double Rad[100][2];
  double run_rad[100];
  while(1){
    getline(ifLL,buf);
    if(buf[0]=='#'){continue;}
    if(ifLL.eof() || lumi_i>25) break;
    stringstream sbuf(buf);
    sbuf >> run_rad[lumi_i] >>  Rad[lumi_i][0] >> Rad[lumi_i][1];
    cout<<"i "<<lumi_i<<" run "<<run_rad[lumi_i]<<" rad0 "<<Rad[lumi_i][0]<<" rad1 "<<Rad[lumi_i][1]<<endl;
    lumi_i++;
  ...
  //  string runlist = "CIGS_Rad.txt";
    string runlist = "CIGS_Rad_all.txt";
  ...
  for(int i=0;i<nch;i++){
    ...
    gRatio_Rad[i] = new TGraphErrors();
    gRatio_Rad[i] -> SetName(Form("gRatio_Rad_%d",i));
    gRatio_Rad[i] -> SetMarkerColor(i+1);
    gRatio_Rad[i] -> SetLineColor(i+1);
    gRatio_Rad[i] -> SetMarkerStyle(2);        
  ...
  while(1){
    ...
    stringstream sbuf(buf);
    sbuf >> rname[s] ;
    RUN[s] = GetRunNum(rname[s]);


    //    cout<<"s "<<s<<" RUN "<<RUN[s]<<" Lumi0 "<<Lumi[s][0]<<" Lumi1 "<<Lumi[s][1]<<endl;
  ...
  unixt[20] = 1669395360; // RUN78 2022/11/26 1:56:00
  unixt[21] = 1669399980; // RUN79 2022/11/26 3:13:00
  unixt[22] = 1669402920; // RUN80 2022/11/26 4:02:00
  unixt[23] = 1669406400; // RUN81 2022/11/26 5:00:00
  unixt[24] = 1669411200; // RUN82 2022/11/26 6:20:00
  unixt[25] = 1669414140; // RUN83 2022/11/26 7:09:00
  unixt[26] = 1669416480; // RUN84 2022/11/26 7:48:00  
  ...
  for(int i=0;i<s;i++){
    ...
    for(int j=0;j<2;j++){
      ...
      //      cout<<"i "<<i<<" j "<<j<<" Lumi "<<Rad[i][j]<<endl;
      gRatio_Rad[j] -> SetPoint(i,Rad[i][j],peak[j][i]/peak[j][0]);
      //      gRatio_Rad[j] -> SetPointError(i,0,(peak[j][i]/peak[j][0])*(peak_err[j][i]/peak[j][i] + peak_err[j][0]/peak[j][0]));
      cout<<"CH "<<j<<" rad "<<Rad[i][j]<<" ratio "<<peak[j][i]/peak[j][0]<<endl;
...
int GetRunNum(string rname){
  vector<string> strvec;
  strvec= split(rname,'_');
  vector<string> strvec2;
  strvec2= split(strvec[1],'.');
  int run = stoi(strvec2[0]);
  return run;
}
...
/*
double GetRad(int run, int ch){

  //  double Rad[100][2];
  //  double run_rad[100];

  for(int i=0;i<100;i++){
    cout<<"i "<<i<<" run "<<run<<" run_rad "<<run_rad[i]<<endl;
    if(run_rad[i]==run){
      return Rad[i][ch];
    }
  }
