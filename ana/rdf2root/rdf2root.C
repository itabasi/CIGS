...
int Read(int RUNNUM=0){
  ...
  */
  ...
  float IntegratedADC2[8] = {0};
  float IntegratedADC_full[8] = {0};
  float sign[8] = {0.0};
  int adc_low[8];
  int adc_high[8];
  float sum[8];
  ...
  int nOver_HWHM[8];
  //  bool AfterPulse[8];
  string new_file = Form("../../rootfiles/run_%03d.root",RUNNUM);
  ...
  tree->Branch("IntegratedADC2",&IntegratedADC2,"IntegratedADC2[8]/F");
  tree->Branch("IntegratedADC_full",&IntegratedADC_full,"IntegratedADC_full[8]/F");
  tree->Branch("sum",&sum,"sum[8]/F");
  tree->Branch("NOver_HWHM",&nOver_HWHM,"nOver_HWHM[8]/I");
  ...
  while(readnum>0){
    ...
    for(int i=0;i<8;i++){
      for(int j =0; j<100;j++){
	IntegratedADC_full[i] += sign[i]*(adc[i][j] - Pedestal[i]);
	if(sign[i]*(PeakHeight[i] + Pedestal[i])/2.0 < sign[i]*adc[i][j] ){
	  nOver_HWHM[i] += 1;
	}
	if(30 <= j  && j <= 50){
	  IntegratedADC[i] += sign[i]*(adc[i][j] - Pedestal[i]);
	  if(sign[i]*adc[i][j] > sign[i]*Pedestal[i])
	    IntegratedADC2[i] += sign[i]*(adc[i][j] - Pedestal[i]);
	}
      }
      //     
      //      for(int j=30;j<50;j++){
