...
void IV_pic(){
  ...
  string name = Form("CIGS%d_IV_woGamma",runnum);
  ...
  string name_g = Form("CIGS%d_IV_wGamma",runnum);
  ...
  c0->cd();
  TGraph*  g = new TGraph();
  g->SetPoint(0,0,1e-10);
  g->SetPoint(1,-3.1,1e-1);
  g->SetTitle("; HV[V] ; Current [A]");
  //  g6->GetYaxis()->SetRangeUser(0,1e-1);
  gPad->SetLogy(1);
  gPad->SetGridy(1);
  gPad->SetGridx(1);
  g->Draw("AP");
